...
var _defaultPatterns = {
	'html': [
      /*jshint regexp:false */
			[/<ng-include[^\>]+src=['"]+([^"']+)["']+/gm,
       'Update the HTML with the new angular include filenames'
			],
       [/<div[^\>]+ng-include=['"]+([^"']+)["']+/gm,
       'Update the HTML with the new angular include filenames inside div'
       ],
    [/<select[^\>]+ng-include=['"]+([^"']+)["']+/gm,
      'Update the HTML with the new angular include filenames inside select'
    ],
    [/<td[^\>]+ng-include=['"]+([^"']+)["']+/gm,
      'Update the HTML with the new angular include filenames inside td'
    ],
      [ /<script.+src=['"]([^"']+)["']/gm,
    ...
    [
      ...
      'Update the CSS to reference our revved images'
		...
		'js': [
			...
			[
			/templateUrl\:\s*['"]?([^'"\)#?]+)(?:[#?](?:[^'"\)]*))?['"]?\s*/gm,
			'Update the JS with the new angular include filenames'
			],
      [
        /TemplateUrl\:\s*['"]?([^'"\)#?]+)(?:[#?](?:[^'"\)]*))?['"]?\s*/gm,
        'Update the JS with the new angular include filenames'
      ...
      [
        /headerUrl\:\s*['"]?([^'"\)#?]+)(?:[#?](?:[^'"\)]*))?['"]?\s*/gm,
        'Update the JS with the proprietary reports template (headerUrl)'
      ],
      [
        /subHeaderUrl\:\s*['"]?([^'"\)#?]+)(?:[#?](?:[^'"\)]*))?['"]?\s*/gm,
        'Update the JS with the proprietary reports template (subHeaderUrl)'
      ]
  ],
  json: [
    [
